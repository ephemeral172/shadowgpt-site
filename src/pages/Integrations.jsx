  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 dark:bg-[#050505] dark:text-white px-6 py-16">
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-xs text-gray-500 dark:text-white/50 mb-4">{t.updated}</p>
        <p className="text-gray-700 dark:text-white/75 font-light mb-10">{t.intro}</p>

        <div className="space-y-8">
          {t.items.map((item, i) => (
            <section key={i}>
              <h2 className="text-lg font-medium text-gray-800 dark:text-white/80 mb-2">{item.name}</h2>
              <p className="text-sm text-gray-700 dark:text-white/75 font-light leading-relaxed">{item.desc}</p>
            </section>
          ))}
        </div>
